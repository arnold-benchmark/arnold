...
class Fusion(nn.Module):
    ...
    def tile_x2(self, x1, x2, x2_proj=None):
        ...
        x2 = x2.unsqueeze(1).unsqueeze(-1).unsqueeze(-1)
        x2 = x2.repeat(1, x1.shape[0]//x2.shape[0], 1, x1.shape[-2], x1.shape[-1])
        x2 = x2.reshape(-1, x2.shape[2], x2.shape[3], x2.shape[4])
        # x2 = x2.unsqueeze(-1).unsqueeze(-1)
        # x2 = x2.repeat(x1.shape[0]//x2.shape[0], 1, x1.shape[-2], x1.shape[-1])
        return x2
...
class FusionConvLat(Fusion):
    ...
    def __init__(self, input_dim=3, output_dim=3):
        ...
        # self.conv = nn.Conv2d(input_dim, output_dim, kernel_size=1, bias=False)
