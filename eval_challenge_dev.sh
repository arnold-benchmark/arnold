...
for i in {1..3}; do
    ...
    for task in "${tasks[@]}"; do
        ...
        /isaac-sim/python.sh eval.py task="$task" model=peract lang_encoder=clip mode=eval use_gt=[0,0] visualize=0 record=True checkpoint_file="$checkpoint_file" output_root="$output_root" data_root=/vagrant/data_for_challenge_val eval_splits=[val]
    done
done